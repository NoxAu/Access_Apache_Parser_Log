...
class Scheduler:
    ...
    def create_scheduler(controller) -> schedule.Scheduler | None:
        ...
        if not schedule_update_logs_enabled:
            print("Automatic log aggregation is disabled\n")
            return None
        else:
            schedule_update_logs_delay = config['app']['schedule_update_logs_delay']
            print(
                f"Logs are aggregated automatically every{schedule_update_logs_delay} seconds. You can disable this in config.yml:app/schedule_update_logs_enabled\n")
